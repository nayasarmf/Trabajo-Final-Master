rm(list=ls())
setwd("~/MASTER/TESIS/codigo_STAD")
...
archivo <- "datos_STAD_CBP_csv.csv"
...
genes_seleccionados <- datos_limpios %>%
  rowwise() %>%
  filter(sum(c_across(-1) > 0) > umbral & any(c_across(-1) >= 2)) %>% # Top genes
  ungroup()
...
write.csv(genes_csv, "genes_seleccionados_positivos_STAD_CBP.csv", row.names = FALSE)

# Paso 5: Leer el archivo creado y buscar nombres de genes en NCBI
archivo_genes <- "genes_seleccionados_positivos_STAD_CBP.csv"
...
write.csv(resultado_final, "genes_seleccionados_positivos_STAD_CBP_nombres.csv", row.names = FALSE)

print("Proceso completado. Archivo 'genes_seleccionados_positivos_STAD_CBP_nombres.csv' creado.")