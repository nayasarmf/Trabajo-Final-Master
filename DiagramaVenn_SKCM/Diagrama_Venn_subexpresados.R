...
generar_diagrama_venn <- function(archivo1, archivo2, columna1, columna2, output_file) {
  ...
  p <- ggVennDiagram(listas, label = "count", label_alpha = 0, edge_size = 0.5) +
    ...
    theme_minimal() +
    labs(title = "Diagrama de Venn Genes SKCM subexpresados")
...
archivo1 <- "genes_seleccionados_negativos_SKCM_CBP_nombres.csv"   # Cambiar por la ruta de tu primer archivo CSV
archivo2 <- "datos_SKCM_GEPIA_underexpressed_csv.csv"   # Cambiar por la ruta de tu segundo archivo CSV
...
output_file <- "diagrama_venn_SKCM_subexpresado.png" # Nombre del archivo de salida
