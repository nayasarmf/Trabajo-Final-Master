...
file_path <- "genes_comunes_negativos_LUAD_log2FoldChange.csv"
...
if (!is.null(ego) && nrow(as.data.frame(ego)) > 0) {
  dotplot(ego, showCategory = 20) + ggtitle("GO Enrichment Analysis LUAD Underexpressed Genes")
  ggsave("GO_Enrichment_Analysis_negativos_LUAD.png", width = 12, height = 12)
  write.csv(as.data.frame(ego), "GO_Enrichment_Results_negativos_LUAD.csv", row.names = FALSE)
} else {
...
if (!is.null(kegg_enrichment) && nrow(as.data.frame(kegg_enrichment)) > 0) {
  dotplot(kegg_enrichment, showCategory = 20) + ggtitle("KEGG Pathway Enrichment LUAD Underexpressed Genes")
  ggsave("KEGG_Pathway_Enrichment_negativos_LUAD.png", width = 10, height = 7)
  write.csv(as.data.frame(kegg_enrichment), "KEGG_Enrichment_Results_negativos_LUAD.csv", row.names = FALSE)
} else {
