...
file_path <- "genes_comunes_negativos_STAD_log2FoldChange.csv"
...
if (!is.null(ego) && nrow(as.data.frame(ego)) > 0) {
  dotplot(ego, showCategory = 20) + ggtitle("GO Enrichment Analysis STAD Underexpressed Genes")
  ggsave("GO_Enrichment_Analysis_negativos_STAD.png", width = 12, height = 12)
  write.csv(as.data.frame(ego), "GO_Enrichment_Results_negativos_STAD.csv", row.names = FALSE)
} else {
...
if (!is.null(kegg_enrichment) && nrow(as.data.frame(kegg_enrichment)) > 0) {
  dotplot(kegg_enrichment, showCategory = 20) + ggtitle("KEGG Pathway Enrichment STAD Underexpressed Genes")
  ggsave("KEGG_Pathway_Enrichment_negativos_STAD.png", width = 10, height = 9)
  write.csv(as.data.frame(kegg_enrichment), "KEGG_Enrichment_Results_negativos_STAD.csv", row.names = FALSE)
} else {
